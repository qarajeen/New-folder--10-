    ...
    const handleViewProject = (project: Project) => {
        if (project.status === 'Awaiting Feedback') {
            const projectWithMedia = {
                ...project,
                media_url: project.project_type.toLowerCase().includes('video') 
                    ? 'https://storage.googleapis.com/gtv-videos-bucket/sample/ForBiggerFun.mp4' 
                    : 'https://picsum.photos/seed/feedback/1200/800',
                media_type: project.project_type.toLowerCase().includes('video') ? 'video' : 'image' as ('video' | 'image'),
            };
            setSelectedProject(projectWithMedia);
                ...
                <div className="bg-eggshell-white/50 dark:bg-deep-ocean-surface/50 p-8 rounded-lg border border-raisin-black/10 dark:border-eggshell-white/10 text-start">
                    ...
                    <div className="mt-6 pt-6 border-t border-raisin-black/10 dark:border-eggshell-white/10 grid grid-cols-1 md:grid-cols-2 gap-8">
                     ...
                     <div className="mt-8 pt-6 border-t border-raisin-black/10 dark:border-eggshell-white/10 flex justify-end">
                        <button onClick={() => setIsEditModalOpen(true)} className="bg-raisin-black/10 dark:bg-eggshell-white/10 text-raisin-black dark:text-eggshell-white font-bold py-2 px-6 rounded-full hover:bg-raisin-black/20 dark:hover:bg-eggshell-white/20 transition">
