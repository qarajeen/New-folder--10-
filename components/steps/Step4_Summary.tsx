...
const GeneratingQuoteAnimation: React.FC = () => (
    <div className="flex justify-center items-center p-8 my-4">
        <svg width="120" height="150" viewBox="0 0 120 150" className="text-action-blue dark:text-vibrant-magenta">
            {/* Page Outline */}
            <path d="M10 1 H110 V149 H10 Z" fill="none" stroke="currentColor" strokeWidth="2" strokeDasharray="1000" strokeDashoffset="1000" className="animate-draw-line" />
...
export const Step4_Summary: React.FC<Step4Props> = ({ formData, onBack, onReset }) => {
  ...
  if (generationState === 'generating') {
    return (
        <div>
            <h2 className="text-2xl font-bold text-center text-raisin-black dark:text-eggshell-white mb-2 uppercase tracking-wider font-serif">
                Putting Your Quote Together...
            </h2>
            <p className="text-center text-raisin-black/70 dark:text-eggshell-white/70 mb-4">Just a moment while we put everything together.</p>
      ...
      <div className="text-center mt-10 no-print flex flex-col items-center gap-4">
        {quote && !error && (
          <div className="interactive-shadow-container w-full sm:w-auto">
            <button onClick={handleExportPDF} className="bg-action-blue text-eggshell-white dark:bg-vibrant-magenta dark:text-raisin-black font-bold py-3 px-8 rounded-full hover:bg-opacity-90 transition transform hover:scale-105 w-full font-sans shine-effect button-inset-shadow">
              Export as PDF
            </button>
          </div>
        )}
        <div className="interactive-shadow-container w-full sm:w-auto">
            <button onClick={onReset} className="bg-transparent border-2 border-action-blue text-action-blue dark:border-vibrant-magenta dark:text-vibrant-magenta font-bold py-3 px-8 rounded-full hover:bg-action-blue/10 dark:hover:bg-vibrant-magenta/10 transition w-full font-sans button-inset-shadow">
                Create a New Quote
            </button>
        </div>
      </div>
      <div className="text-center mt-6">
        <button onClick={onBack} className="text-sm font-semibold text-raisin-black/60 dark:text-eggshell-white/60 hover:text-action-blue dark:hover:text-vibrant-magenta transition">
